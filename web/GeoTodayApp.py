...
incidents['ResponseDate'] = pd.to_datetime(incidents['ResponseDate'], format='%m/%d/%Y %I:%M:%S %p')
...
filters_layout = html.Div([
    ...
    dcc.Dropdown(
        id='year-filter',
        options=[{'label': year, 'value': year} for year in sorted(incidents['Year'].unique())],
    ...
    dcc.Dropdown(
        id='month-filter',
        options=[{'label': month, 'value': month} for month in sorted(incidents['Month'].unique())],
    ...
    dbc.Checklist(
        id='problem-filter',
        options=[{'label': 'Select All', 'value': 'All'}] + [{'label': problem, 'value': problem} for problem in problems],
