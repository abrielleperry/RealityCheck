...
data['datetime'] = pd.to_datetime(data['datetime'], format='%Y-%m-%d %I:%M:%S %p', errors='coerce')

# Handling missing values
imputer = SimpleImputer(strategy='most_frequent')
data[['incident', 'action_taken']] = imputer.fit_transform(data[['incident', 'action_taken']])
...
incident_encoded_df = pd.DataFrame(incident_encoded, columns=encoder_incident.get_feature_names_out(['incident']))
action_taken_encoded_df = pd.DataFrame(action_taken_encoded, columns=encoder_action_taken.get_feature_names_out(['action_taken']))
...
X_train, X_test, y_train, y_test = train_test_split(daily_features, daily_incidents, test_size=0.2, random_state=42)
